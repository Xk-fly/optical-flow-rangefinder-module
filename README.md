...
2. 只有VL53完成真实读取、距离小于50mm，且ULD RangeStatus为0（有效）或3（最小量程裁剪）时，才发送50mm（5cm）的地面启动距离；
...
因此虚拟值只解决“传感器正常但起飞前距离低于其可靠量程”的启动问题；I2C/API故障以及Signal/Phase/Hardware/Processing/Invalid Range等异常状态都不会触发虚拟距离，也不会掩盖飞行中的测距失效。地面启动阶段会发送一次 `VL53 ground bootstrap 5cm`，切入真实测距后会发送一次 `VL53 real range locked`，便于从飞控日志确认状态切换。
